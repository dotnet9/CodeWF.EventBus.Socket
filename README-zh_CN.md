...
`CodeWF.EventBus.Socket` 是一个轻量级的、基于 Socket 的分布式事件总线系统，旨在简化分布式架构中的事件通信。它允许进程之间通过发布/订阅（Pub/Sub）模式进行通信，无需依赖外部消息队列服务。通过本库，您可以轻松实现微服务间的同步调用（CQRS中的Command）和异步事件通知。
