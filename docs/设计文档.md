...
| `CodeWF.NetWrapper` | 提供 `TcpSocketServer`、`TcpSocketClient`、实例命令处理器以及共享传输对象 |
...
- 直接复用 `CodeWF.NetWrapper`：`TcpSocketServer`、`TcpSocketClient`、`RegisterCommandHandler`、`SocketCommand`、`Heartbeat`
...
- `EventServer` 基于 `TcpSocketServer` 运行，并通过服务端实例的 `RegisterCommandHandler` 接收入站命令。
- `EventClient` 基于 `TcpSocketClient` 运行，并通过客户端实例的 `RegisterCommandHandler` 接收服务端响应。
- 普通事件总线命令不再经过全局 `CodeWF.EventBus.Default` 做转发和端点过滤；客户端断线错误仍兼容 `TcpClientErrorCommand`。
- 心跳连续失败超过阈值后，客户端会尝试重连。
- 客户端和服务端的连接生命周期绑定到独立 session，支持取消、幂等停止和资源释放。
- 客户端重连握手成功后会重新注册本地主题订阅。
- 服务端按客户端隔离有界发送队列，单个慢客户端不会阻塞其他客户端。
- 可通过 `EventBusOptions.AuthenticationToken` 启用共享令牌握手认证。
...
- 查询映射具有服务端过期时间，客户端也支持 `CancellationToken` 主动取消。
- 主题长度、消息体大小、入站队列和出站队列均有可配置上限。

## 当前限制

- 不提供持久化或服务端可靠队列
- 共享令牌认证是应用层握手保护，不提供 TLS 加密；跨不可信网络仍需在传输层配置 TLS 或专用安全隧道。
- 不支持多服务端实例之间的共享状态
- 不内置指标、链路追踪和死信处理
